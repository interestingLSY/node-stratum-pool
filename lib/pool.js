...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            ...
            emitLog('p2p peer node disconnected - attempting reconnection...');
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (result.error) {
						...
						emitErrorLog('将块提交到 daemon 的时候出现错误！result: '+JSON.stringify(result.response));
					}
                }
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function () {
                    CheckBlockAccepted(shareData.blockHash, function (isAccepted, rpcData, tx) {
						if(!isAccepted){
							emitErrorLog('我们认为我们已经找到了一个块，但它被 daemon 拒绝了');
							emitErrorLog('shareData: '+JSON.stringify(shareData));
							emitErrorLog('rpcData: '+JSON.stringify(rpcData));
						}
    ...
    function StartStratumServer(finishedCallback) {
        ...
        }).on('clientDisconnected', function(client,subscriptionId){
			_this.emit('clientDisconnected',client,subscriptionId);
		});
    }
    ...
    function CheckBlockAccepted(blockHash, callback) {
        ...
        _this.daemon.cmd('getblock',
            ...
            function (results) {
                ...
                if (validResults.length >= 1 && validResults[0].response.confirmations >= 0 ) {
                    callback(true, validResults, validResults[0].response.tx[0]);
                }
                else {
                    callback(false, validResults);
                }
            }
        );
        //}, 500);
