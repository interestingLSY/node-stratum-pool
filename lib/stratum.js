...
var SubscriptionCounter = function(){
    ...
    return {
        next: function(){
            var id = Math.floor(Math.random()*(2**63+10));
            return padding + util.packInt64LE(id).toString('hex');
        }
    };
};
...
var StratumClient = function(options){
    ...
    function handleAuthorize(message, replyToSocket){
		...
		if( _this.workerName.indexOf('.') != -1 ){
			var t = _this.workerName.indexOf('.');
			_this.address = _this.workerName.substr(0,t);
			_this.label = _this.workerName.substr(t+1);
		}else{
			_this.address = _this.workerName;
			_this.label = _this.workerPass || '未命名';
		}
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, options.subscriptionId, _this.address, _this.label, function(result) {
 ...
 *   - 'clientConnected'(StratumClientInstance) - when a new miner connects
 *   - 'clientDisconnected'(StratumClientInstance,subscriptionId) - when a miner disconnects. Be aware that the socket cannot be used anymore.
