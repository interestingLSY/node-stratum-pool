...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask, subscriptionId) {
        ...
        var headerHash = hashDigest(headerBuffer, nTimeInt, job.prevHash);
        ...
        blockHashInvalid = blockHasher(headerBuffer, nTime, job.prevHash).toString('hex');

        //Check if share is a block candidate (matched network difficulty)
        if (job.target.ge(headerBigNum)) {
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nTime, job.prevHash).toString('hex');
        }
        else {
            ...
            if (shareDiff < difficulty*0.99) {
                ...
                if (previousDifficulty && shareDiff >= previousDifficulty*0.99 ) {
		...
		var blockReward;
		if(!job.rpcData.masternode){
			blockReward = job.rpcData.coinbasevalue;
		}else{
			blockReward = job.rpcData.coinbasevalue - job.rpcData.masternode.amount;
		}
        _this.emit('share', {
            job: jobId,
            ip: ipAddress,
