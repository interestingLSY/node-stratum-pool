...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, network, poolOptions) {
	...
	// 某些货币强制要求 founder reward
	if(rpcData.founder_reward_enforced){
		var founderReward = rpcData.founderreward.founderamount;
		reward -= founderReward;
		rewardToPool -= founderReward;
		var founderPayeeScript = util.addressToScript(network,rpcData.founderreward.founderpayee);
		txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(founderReward),
            util.varIntBuffer(founderPayeeScript.length),
            founderPayeeScript
        ]));
	}
